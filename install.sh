...
main() {
    ...
    [ "$(uname -s)" = Linux ] || fail 'Release binaries support Linux only.'
    case "$(uname -m)" in
        x86_64|amd64) arch=amd64 ;;
        aarch64|arm64) arch=arm64 ;;
        *) fail 'Supported architectures: x86_64 (amd64) and aarch64 (arm64).' ;;
    esac
    for tool in curl sha256sum mktemp awk mkdir cp chmod mv rm; do
    ...
    printf 'Downloading Systemdoc %s for Linux %s…\n' "$version" "$arch"
