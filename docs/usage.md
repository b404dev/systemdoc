...
**Systemd** combines loaded services and installed unit files, separates enablement from runtime state, and shows available CPU/memory accounting. The inspector provides status, streaming journal, source configuration, sampled metrics, and dependencies. The Metrics tab and the selected-service band are filled from the one bulk `systemctl show` that already samples the list: CPU and memory trails, tasks, peak memory, cumulative I/O and the restart count, which the band shows beside boot enablement whenever the manager has restarted the unit. Runtime aliases such as `dbus.service` resolve to their canonical service; searching an alias finds that service without duplicating its active count. Template files (`name@.service`) are labelled separately and require a named instance for runtime inspection. An installed file absent from the runtime snapshot is labelled `unknown`, not guessed inactive or unloaded. The compact selected-service band shows runtime state/substate, boot enablement and resources; Overview retains load state and the full description. Toggle system/user scope with `u`. Press `i` or click **Active only** to hide inactive services; the systemd choice persists between sessions. `--active-only` enables the filter at launch, including over SSH.

**Docker** shows observed containers with state and available CPU/memory statistics. The selected-workload card and expanded table expose health/status, image, and Compose project membership. The Overview tab shows container name and ID, image, state and health, restart policy, Compose membership, host/container ports, volumes and bind mounts, attached networks and addresses, runtime options, limits, and labels. Published ports and exposed-only ports are distinguished; configured bindings without a live mapping are labeled. Mounts include access mode, source, and destination. Environment values remain in the full inspect JSON (`c`), rather than the overview. `d` opens Connections with ports, mounts, networks, and labels; `l` streams logs and `r` shows the live resource statistics (CPU, memory, network and block I/O, processes) from the same `docker stats` sample that fills the list, so the tab costs no extra command. Structured Docker views wrap long paths and values; `z` expands them. Press `p` for Compose projects, including registered projects whose containers are down.
...
Four Pulse cards show active workloads, those needing attention, host CPU and host memory. After a second sample, all four become real history charts with the latest direction shown beside the value. Click Active or Attention to filter the list; click again to clear. `A` cycles the same state filters from the keyboard. The always-visible **i Active only** button directly toggles active/running workloads. Each mode keeps its own filter when you switch. Counts describe the current backend inventory. The two host cards read the whole machine on a fixed 0–100% scale and name the tracked workload sum beside it, which includes only workloads reporting counters. The CPU card also carries the one-minute load average and the memory card names swap in use. From 120 columns the CPU card's title names how many logical CPUs the machine has and the clock they are running at (`HOST CPU · 8 CPUs · 3.2 GHz`, or a spread such as `800 MHz–4.2 GHz` when cores disagree), and the masthead repeats the same count and clock after the CPU share. The masthead also repeats memory and load, and adds a `STALL` reading from the kernel's pressure accounting when tasks have been waiting for CPU, memory or I/O for more than 5% of the last ten seconds. On Linux the clock is cpufreq's current frequency where the kernel exposes it and otherwise `/proc/cpuinfo`'s figure, which most virtual machines hold fixed; on Apple Silicon no clock is available and only the count is shown. Press `C` or click the HOST CPU card for **CPU Cores**, a per-CPU table with each logical CPU's busy share, user/system/iowait/steal split, clock, core and socket and trend; from Process Explorer (`4`, then `c`) it also names the busiest process on each CPU. [Host panels](host-panels.md#process-explorer) describes the view, the per-CPU strip on the Process Explorer card and the sources in detail. CPU uses one logical CPU as 100% and may exceed it. Missing values stay `—`; a non-zero reading is always drawn as at least one level. When an accounting command fails (for example `docker stats` without permission) the summary line says `accounting unavailable` and names the command, so blank columns are never mistaken for an idle machine.
