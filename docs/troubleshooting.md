...
If `systemdoc` is not found after installation, add the install directory to PATH or run `~/.local/bin/systemdoc` directly. Linux amd64/arm64 are the distributed targets; macOS, Windows, and 32-bit ARM do not have release binaries.
...
Start with a true-color terminal, standard Unicode monospace font, and Deep Navy (`t`). Clear accent/background overrides under Preferences. Retired theme names fall back to Deep Navy. Nerd Font icons are optional and require the font in your local terminal, including over SSH.
...
`--upload` requires a matching static Linux ELF executable. A `/tmp` mounted with `noexec` prevents execution: install remotely in an executable location and use `--remote-bin`. Host-key and authentication prompts belong to OpenSSH. See [remote hosts](remote.md).
